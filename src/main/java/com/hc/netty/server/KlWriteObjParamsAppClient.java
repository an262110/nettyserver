 ...
 * 写对象参数(命令0x08)
 */
public class KlWriteObjParamsAppClient {
	private static KlWriteObjParamsAppClient idnAppClient = new KlWriteObjParamsAppClient();
	...
	public static KlWriteObjParamsAppClient getInstance() {
		return idnAppClient;
	}
	public static void main(String[] args){
		//222222111111
		//111111222224
		String ret= KlWriteObjParamsAppClient.getInstance().sendPre("111111222228", "2","10000002");
	 ...
	 * @param gun_code_no 枪生成二维码的编号
	 * @return
	 */
	public String sendPre(String ljaddress,String gunNo,String gun_code_no){
		String ret = null;
...
//		byte[] lj_address = ToolUtil.bytesReverseOrder(ljbyteaddress);
		...
		//控制码0xB8
		byte[] ret_c = new byte[]{(byte) 0xB6};

		/**
		 * 测量点号（TN） 1-255表示
		 */
		String hen_gun_no = ToolUtil.dec2HexString(gunNo);
		if(hen_gun_no.length() == 1){
			hen_gun_no = "0"+hen_gun_no;
		}
		/**
		 * 数组形式的测量点号
		 */
		byte[] ret_gun_no = ToolUtil.hexStringToBytes(hen_gun_no);


		//权限等级
		byte[] role_bytes = new byte[]{(byte) 0x11};

		//密码（PW）
		byte[] pwd_bytes = ToolUtil.fill0x00(3);
...
//		byte[] bytes_gun = ToolUtil.hexStringToBytes(gun_no_hex);
		 ...
		 * 处理好的数据编号
		 */
		byte[] bianhao_bytes = ToolUtil.dealData(gun_code_no);

		List<byte[]> length_list = new ArrayList<byte[]>();
		length_list.add(ret_gun_no);
		length_list.add(role_bytes);
		length_list.add(pwd_bytes);
		length_list.add(bianhao_bytes);
		byte[] bytes_length_all = ToolUtil.appendByte(length_list);
		byte[] ret_length = ToolUtil.intToBytes(bytes_length_all.length, 2, 0);
		...
		content_list.add(ret_gun_no);
		content_list.add(role_bytes);
		content_list.add(pwd_bytes);
		content_list.add(bianhao_bytes);
