...
public class ISOUtil {
    ...
    */
    ...
    /**
     * 将数组从头到尾进行位置兑换 如 1,2,3 换成 3,2,1
     * @param rtua
     * @return
     */
    public static byte[] arrayOrderDesc(byte[] rtua) {
        for (int start = 0, end = rtua.length - 1; start < end; start++, end--) {
            byte temp = rtua[end];
            rtua[end] = rtua[start];
            rtua[start] = temp;
        }
